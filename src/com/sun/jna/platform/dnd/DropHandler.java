...
public abstract class DropHandler implements DropTargetListener {
   ...
   protected int getDropAction(DropTargetEvent e) {
      ...
      if(e instanceof DropTargetDragEvent) {
         DropTargetDragEvent availableActions = (DropTargetDragEvent)e;
         currentAction = availableActions.getDropAction();
         sourceActions = availableActions.getSourceActions();
         flavors = availableActions.getCurrentDataFlavors();
         location = availableActions.getLocation();
      } else if(e instanceof DropTargetDropEvent) {
         DropTargetDropEvent availableActions1 = (DropTargetDropEvent)e;
         currentAction = availableActions1.getDropAction();
         sourceActions = availableActions1.getSourceActions();
         flavors = availableActions1.getCurrentDataFlavors();
         location = availableActions1.getLocation();
      }

      if(this.isSupported(flavors)) {
         int availableActions2 = this.getDropActionsForFlavors(flavors);
         currentAction = this.getDropAction(e, currentAction, sourceActions, availableActions2);
   ...
   protected boolean isSupported(DataFlavor[] flavors) {
      HashSet set = new HashSet(Arrays.asList(flavors));
