...
public abstract class DragHandler implements DragSourceListener, DragSourceMotionListener, DragGestureListener {
   ...
   protected DragHandler(Component dragSource, int actions) {
      ...
      try {
         String src = System.getProperty("DragHandler.alpha");
         if(src != null) {
            try {
               this.ghostAlpha = Float.parseFloat(src);
      ...
      DragSource src1 = DragSource.getDefaultDragSource();
      src1.createDefaultDragGestureRecognizer(dragSource, this.supportedActions, this);
   }
   ...
   public void dragGestureRecognized(DragGestureEvent e) {
      if((e.getDragAction() & this.supportedActions) != 0 && this.canDrag(e)) {
         ...
         try {
            ...
            Icon icon = this.getDragIcon(e, ex);
            Point origin = e.getDragOrigin();
            this.imageOffset = new Point(ex.x - origin.x, ex.y - origin.y);
            Icon dragIcon = this.scaleDragIcon(icon, this.imageOffset);
            Object cursor = null;
            if(dragIcon != null && DragSource.isDragImageSupported()) {
               GraphicsConfiguration screen1 = e.getComponent().getGraphicsConfiguration();
               e.startDrag((Cursor)cursor, this.createDragImage(screen1, dragIcon), this.imageOffset, transferable, this);
            } else {
               if(dragIcon != null) {
